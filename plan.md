...
## Hero background animation — try 4 variants sequentially (new addition)

Replace the current drifting-grid hero background (`frontend/src/pages/Landing.jsx` `.hero-bg` + matching CSS in `index.css`) with each of these 4 candidates one at a time, showing the user after each so they can pick a favorite before moving to the next:

1. **Aurora mesh glow** — 2-3 large soft-blurred gradient blobs (indigo/cyan) slowly drift and morph behind the existing faint grid. Pure CSS keyframes.
2. **Scanning beam / radar sweep** — a thin diagonal soft-edged light band sweeps across the grid on a slow loop.
3. **Cursor-reactive glow** — a radial glow follows the mouse position over the grid; small `mousemove` listener in `Landing.jsx`, CSS transition for easing (only variant needing JS).
4. **Slow conic gradient rotation** — one large soft conic/pinwheel gradient behind the grid rotates back and forth a small arc over a long loop.

Only `.hero-bg` (and its keyframes) in `index.css`, plus `Landing.jsx` for variant 3's mouse listener, are touched — nothing else in the hero (headline, CTA, grid) changes. Each variant fully replaces the previous one's CSS/markup so only one is live at a time.

**Decision: variant 2 (scanning beam / radar sweep) was chosen** and is now the permanent hero background. Variants 1/3/4 code was removed; `Landing.jsx` has no mouse-tracking JS (only variant 3 needed it).

**Follow-up: replaced the flat square-grid backdrop under the beam** with 4 alternative textures shown one at a time (perspective grid floor, topographic contour lines, circuit-trace pattern, fine noise/grain) — **circuit-trace pattern was chosen** (SVG data-URI tile of right-angle PCB-style traces with glowing node dots, `index.css` `.hero-bg` background-image) and is now permanent, paired with the beam-sweep `::before`.

## Buyer/Seller audience separation (new addition)

Business need: contacts are either buyers or sellers, and calls should be organized by which. Chose the "tag + filter" approach (not fully separate nav sections): added an `audience` column (`buyer`/`seller`, `models.CallAudience`) to `calls` via migration `0003_add_audience.py`, required on `POST /api/calls`. Frontend: `components/AudienceToggle.jsx` (Buyer/Seller picker) added to both New Call and Scheduler forms; `components/AudienceTabs.jsx` (All/Buyers/Sellers filter) added to Dashboard and Scheduler's upcoming-calls list, filtering both the stats and the table. No separate default/saved script per audience — script is still typed fresh each time regardless of audience (explicitly deferred).

## Script personalization placeholders + organization field (new addition)

Added `organization` (nullable) to `calls`/`Recipient`/`ParsedRecipient` (migration `0002_add_organization.py`) — auto-detected from Excel/CSV column aliases (organization/company/org/business/employer) and PDF table middle-column, plus a manual field in `components/RecipientsEditor.jsx`. Script text supports `{{name}}` and `{{company}}` placeholders, substituted per-recipient at call time in `routers/voice.py`'s `_render_script()` (used by the `/voice/twiml/{id}` webhook) — `components/ScriptEditor.jsx` adds "Insert placeholder" buttons at the cursor position.

## Switched telephony provider: Twilio → Plivo (new addition)

Since this app is India-only and Twilio doesn't sell Indian local phone numbers (a real blocker for buyer/seller outreach), the backend was fully migrated to **Plivo**, which supports Indian voice-enabled numbers with self-serve signup. Change was contained: `backend/app/twilio_client.py` deleted, replaced by `backend/app/plivo_client.py` (Plivo REST `calls.create(answer_url=, hangup_url=)`); `routers/voice.py` rewritten to serve Plivo XML (`<Speak>` instead of `<Say>`) at new `/voice/answer/{id}` and `/voice/hangup/{id}` routes (Plivo's hangup callback carries `HangupCause`, mapped to `completed`/`failed`); `config.py` env vars renamed `PLIVO_AUTH_ID`/`PLIVO_AUTH_TOKEN`/`PLIVO_FROM_NUMBER`; `calls.twilio_call_sid` column renamed to `provider_call_id` via migration `0005_rename_call_sid_to_provider_call_id.py`. `twilio.md` replaced by `plivo.md` (setup steps, required DLT/KYC documents, India-specific pricing, final checklist). No other part of the app (scheduling, dashboard, script placeholders, buyer/seller audience) needed to change.
